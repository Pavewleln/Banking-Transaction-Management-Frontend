...
import {useDeleteCardMutation} from "../../store/cards/cards.api";
import {toast} from "react-toastify";
import {useNavigate} from "react-router-dom";

export const DeleteCardPopup: FC<IPopup> = ({showModal, setShowModal, cardDetails}) => {
    const navigate = useNavigate()
    const [deleteCard, {
        isSuccess: isDeleteCardSuccess,
    ...
    useEffect(() => {
        if (isDeleteCardSuccess) {
            setShowModal(false)
            toast.success("Карта успешно заблокирована")
        }
    }, [isDeleteCardSuccess])

    // Если ошибка
    useEffect(() => {
        if (isDeleteCardError) {
            toast.error((deleteCardError as any).data.message)
        }
    }, [isDeleteCardError])

    const onSubmit = async (cardId: string | undefined) => {
        try {
                            ...
                            <div className="mt-3">
                                ...
                                <div className="mt-2 text-center sm:text-left">
                                    <h4 className="text-lg font-medium text-gray-800 text-center dark:text-white">
                                        Вы точно хотите заблокировать карту?
                                    </h4>
                                    <div className="items-center gap-2 mt-3 sm:flex">
                                        <button
                                            ...
                                            onClick={() => onSubmit(cardDetails?._id)}
                                        >
                                            Заблокировать
                                        </button>
                                        <button
